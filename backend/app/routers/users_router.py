...
def create_user(
    ...
    db.add(u)
    db.add(AuditLog(actor_email=admin.email, action="CREATE_USER", target=body.email))
    db.commit()
    db.refresh(u)
    return _to_resp(u)
...
def update_user(
    ...
    db.add(AuditLog(actor_email=admin.email, action="UPDATE_USER", target=str(user_id)))
    db.commit()
    db.refresh(u)
    return _to_resp(u)
